...
-  a contract is no good if there is no enforcement. Configure the Spring Cloud Contract Maven Verifier plugin on the producer API build. You'll need to specify what base class (or classes) should be used to setup the tests, too. This plugin is what translates our contract into a test on compilation. If the contract (and thus the test) don't match, then the build will fail. Our contract will prevent us from shipping an API that violates the assumptions of the contract!
  ...
  - Now when you run `mvn clean install` the contract will ensure that your APIi hasn't changed in an unexpected way.
  - When a build succeeds, with `mvn clean install`, the build contributes a `.pom`, `.jar` and `-stub.jar`. This last artifact contains the definition from our contract that we care about. it is this stub that we'll use with our client.
  ...
  - As configured, the client test will spin up a WireMock-based API that's pre-programmed to respond according to the contract. It'll run for the life of the client test, and no longer. It's an actual HTTP API, against which we can make client-side invocations. It does not, however, need all the supporting infrastructure to work. It just works.
